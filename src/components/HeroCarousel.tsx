...
const HeroCarousel = () => {
  ...
  return <section className="relative pt-24 pb-8">
      <div className="container mx-auto px-4">
        <div className="mb-8 text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Acompanhe as últimas tendências e notícias das principais plataformas em um só lugar.</h2>
        ...
        <div className="relative max-w-6xl mx-auto">
          <div className="relative overflow-hidden rounded-2xl">
            ...
            transform: `translateX(-${currentSlide * 100}%)`
              ...
              {highlights.map(highlight => <div key={highlight.id} className="w-full flex-shrink-0">
          ...
          {/* Navigation Buttons */}
          <button onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-black/50 backdrop-blur-sm border border-white/20 hover:bg-orange-500/20 hover:border-orange-500/40 transition-all duration-300">
            <ChevronLeft className="w-6 h-6" />
          </button>
          
          <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-black/50 backdrop-blur-sm border border-white/20 hover:bg-orange-500/20 hover:border-orange-500/40 transition-all duration-300">
            <ChevronRight className="w-6 h-6" />
          </button>

          {/* Dots Indicator */}
          <div className="flex justify-center space-x-2 mt-6">
            {highlights.map((_, index) => <button key={index} onClick={() => setCurrentSlide(index)} className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentSlide ? "bg-orange-500 w-8" : "bg-white/30"}`} />)}
          </div>
        </div>
      </div>
    </section>;
